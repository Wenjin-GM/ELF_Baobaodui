...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        self.stacked_widget.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Ignored)
    ...
    def _create_top_bar(self) -> QWidget:
        ...
        # 系统名称
        title_label = QLabel("智能工具柜")
        title_label.setFixedWidth(210)
        ...
        self.state_label.setFixedWidth(350)
        ...
        self.user_label.setMaximumWidth(160)
    ...
    def _create_bottom_nav(self) -> QWidget:
        """创建底部导航栏"""
        nav_bar = QFrame()
        nav_bar.setFixedHeight(BOTTOM_NAV_HEIGHT)
    ...
    def _create_pages(self):
        ...
        dashboard.setMinimumSize(0, 0)
        dashboard.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Ignored)
        self.pages[PageType.DASHBOARD] = dashboard
        self.stacked_widget.addWidget(dashboard)

        # 认证页
        auth = AuthPage(self.state_machine, self.backend)
        auth.setMinimumSize(0, 0)
        auth.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Ignored)
        ...
        tools.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Ignored)
        self.pages[PageType.TOOLS] = tools
        self.stacked_widget.addWidget(tools)

        # 充电页
        charging = ChargingPage(self.state_machine, self.backend)
        charging.setMinimumSize(0, 0)
        charging.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Ignored)
        ...
        environment.setMinimumSize(0, 0)
        environment.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Ignored)
        ...
        records.setMinimumSize(0, 0)
        records.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Ignored)
        ...
        settings.setMinimumSize(0, 0)
        settings.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Ignored)
        ...
        debug.setMinimumSize(0, 0)
        debug.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Ignored)
    ...
    def _on_state_changed(self, old_state: SystemState, new_state: SystemState):
        ...
        if user:
            role_text = "管" if user["role"] == "admin" else "用户"
