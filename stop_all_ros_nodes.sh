...
if [[ -d "$PID_DIR" ]]; then
  for name in ui_node cabinet_logic_node face_node nfc_node actuator_node env_node battery_node vision_node; do
...
pkill -f '/smart_cabinet_nodes/lib/smart_cabinet_nodes/vision_node' 2>/dev/null || true
...
pkill -f 'ros2 run smart_cabinet_nodes vision_node' 2>/dev/null || true
